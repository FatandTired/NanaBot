...
const off = new db.crearDB('offed')
   ...
   if(verificaroff) return;
    ...
    if(user.id === message.member.id) return message.channel.send('No puedes silenciarte a ti mismo')
    
    if(message.member.roles.highest.position < user.roles.highest.position) return message.channel.send('No puedes silenciar miembros por encima de ti')
    
    let verificar5 = toggl.some('developers', (v) => v == user.id)
    if(verificar5) return message.channel.send('No puedes silenciar encargados')
    if(!verificar5){
    const trusted = new db.crearDB('trusted')
    let verificartrsted = trusted.some(server, (v) => v == user.id)
    if(verificartrsted) return message.channel.send('No puedes silenciar encargados')
    }
    let verificar1 = toggle.some(server, (v) => v == user.id)
    if(verificar1) return message.channel.send('El usuario ya se encuentra silenciado')
    ...
    if(!verificar1){
       ...
       message.channel.send('No se pudo establecer el usuario, asegurate que se encuentre dentro del servidor')
        ...
        message.channel.send('El usuario a sido silenciado')
    }
